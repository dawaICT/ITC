...
if (isset($_POST["submit"]) && isset($_FILES["file"])) {
    ...
    if (!$file || !is_uploaded_file($file)) {
        ...
        wuc_safe_redirect('medicine.php');
    }
    ...
    while (($csv = fgetcsv($file_open, 10000, ",")) !== false) {
        ...
        foreach (['A1' => $A1, 'A2' => $A2, 'T1' => $T1, 'T2' => $T2] as $label => $mark) {
            if ($mark < 0 || $mark > 100) {
                $results_log[] = ['type' => 'warning', 'msg' => "Skipped $Sid: $label must be between 0 and 100."];
                $skipped++; continue 2;
            }
        }
        ...
        if (!$periodCheck['ok']) {
            $results_log[] = ['type' => 'warning', 'msg' => "Skipped $Sid: " . $periodCheck['message']];
            $skipped++; continue;
        }
        ...
        if (!$elig['allowed']) {
            $results_log[] = ['type' => 'warning', 'msg' => "Skipped $Sid: Paid " . $elig['percent'] . "% (min 50%)."];
            $skipped++; continue;
        }

        $save = ca_save_components($db, $Sid, $Course_Code, $semester, $Year, 'medicine', [
            'A1' => $A1,
            'A2' => $A2,
            'T1' => $T1,
            'T2' => $T2,
        ...
        if ($save['ok']) {
            ...
            $results_log[] = ['type' => 'warning', 'msg' => "Skipped $Sid: " . $save['message']];
            $errors++;
        }
    }
    ...
    wuc_flash('success', "Medicine CA Upload: $success processed, $skipped skipped, $errors errors.");
...
<div class="container-fluid px-4 portal-dashboard">
    <div class="dashboard-header admin-section mb-4">
        <div class="row align-items-center">
            <div class="col">
                <h1 class="dashboard-title">Medicine CA Upload</h1>
                <p class="text-muted">CSV Upload for Clinical Medicine programme marks</p>
            </div>
            <div class="col-auto">
    ...
    <div class="card shadow">
        <div class="card-header"><h5 class="mb-0"><i class="fas fa-file-csv me-2"></i>Upload Medicine CA CSV</h5></div>
        <div class="card-body">
            <div class="alert alert-info">
                <i class="fas fa-info-circle me-2"></i>
                <strong>Medicine CA:</strong> Enter A1, A2, T1, and T2 as local CA marks out of 100. External examination marks are not uploaded here.
            </div>
