 ...
 * portals, portal_code 'alumni'), the same mechanism portal_selection.php
 * uses to show the Alumni tile. A legacy $_SESSION['role'] of 'alumni' or
...
if (!defined('IS_SCRIPT')) {
    ...
    if (empty($_SESSION['logged_in']) && empty($_SESSION['Sid'])) {
        $_SESSION['errorMessage'] = 'Please log in to access the Alumni Portal.';
        header('Location: /wucportal/staff_login.php', true, 303);
        exit;
    }

    $legacyRole = (string)($_SESSION['role'] ?? '');
    if ($legacyRole === 'alumni' || $legacyRole === 'systems_admin') {
        wuc_resolve_session_user_id($db); // backfill Sid/user_id_db when mapped
        $_SESSION['current_portal'] = 'alumni';
    } else {
        // Denials are audit-logged and redirected to portal_selection.php.
        wuc_require_portal_access($db, 'alumni', 'You do not have permission to access the Alumni Portal.');
    }
}
