...
wuc_enforce_session_guard([
    ...
    'login_message' => 'Please log in to access the Registrar Portal.',
]);
...
function itc_academic_report_csv(string $filename, array $filters, array $report): void
{
    ...
    if (isset($db) && $db instanceof mysqli && function_exists('audit_log_current_user')) {
        audit_log_current_user($db, 'reports.registrar_academic.export', [
            'filename' => $filename,
            'filters' => $filters,
...
$hasRegistrarAccess = hasRole(ROLE_REGISTRAR)
    || hasRole(ROLE_SYSTEMS_ADMIN)
    || (function_exists('isSystemsAdmin') && isSystemsAdmin());
$hasGlobalReportScope = $hasRegistrarAccess && itc_academic_report_is_global_viewer($staffId);
$canViewReports = $hasRegistrarAccess && ($hasGlobalReportScope
...
if (!$canViewReports) {
    wuc_permission_denied('You do not have permission to view registrar academic reports.', '/wucportal/portal_selection.php');
}
...
if ($reportGenerated) {
    try {
        ...
        error_log('registrar/academic_reports.php: report generation failed: ' . $e->getMessage());
...
render_report_print_header('Registrar Academic Report', 'TEVETA-aligned academic structure summary', $printMeta);
?>

<div class="container-fluid px-4 portal-dashboard">
    <div class="dashboard-header registrar-section mb-4 d-print-none">
        <div>
            <h1 class="dashboard-title">
                <i class="fas fa-chart-pie me-2"></i>Academic Reports
            </h1>
            <p class="text-muted mb-0">Generate registrar-wide section, department, programme, CA, results, lecturer workload, registration and eLearning reports from normalized academic data.</p>
        </div>
        <div class="header-actions">
            ...
            <?php endif; ?>
            <button type="button" class="btn btn-outline-secondary rounded-pill px-3" onclick="printReport('Registrar Academic Report')" <?php echo !$reportGenerated ? 'disabled' : ''; ?>>
