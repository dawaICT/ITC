...
// Read-only, explainable view of the latest rule-based risk score per student.
// Scores are produced by includes/academic_risk_engine.php (page-triggered) and
// scripts/batch_risk_rescore.php (scheduled batch); nothing is decided here.

$riskTableReady = wuc_risk_table_exists($db, 'student_risk_summary');
...
if ($riskTableReady) {
    try {
        // Latest snapshot per student (the engine appends one row per rescore).
        ...
        error_log('registrar/risk_watchlist.php query failed: ' . $e->getMessage());
        $rows = [];
    }
}
?>
                ...
                <table class="table table-sm align-middle mb-0">
                    ...
                    <tbody>
                        <?php foreach ($rows as $row): ?>
                            <?php
                            $cls = wuc_academic_risk_level_class((string)$row['risk_level']);
                            $reasons = array_filter(array_map('trim', explode("\n", (string)($row['risk_reason'] ?? ''))));
                            ?>
                            <tr>
                                <td>
                                    <strong><?= htmlspecialchars(trim((string)($row['student_name'] ?? '')) ?: (string)$row['student_id']) ?></strong><br>
                                    <small class="text-muted"><?= htmlspecialchars((string)$row['student_id']) ?></small>
                                </td>
                                <td><?= htmlspecialchars((string)($row['program_code'] ?? '')) ?></td>
                                <td><span class="badge bg-<?= htmlspecialchars($cls) ?>"><?= htmlspecialchars((string)$row['risk_level']) ?></span></td>
                                <td class="fw-bold text-<?= htmlspecialchars($cls) ?>"><?= (int)$row['risk_score'] ?></td>
                                <td>
                                ...
                                <td class="small"><?= htmlspecialchars((string)($row['recommended_action'] ?? '')) ?></td>
                                <td class="small text-muted"><?= htmlspecialchars((string)($row['generated_at'] ?? '')) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
