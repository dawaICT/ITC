...
if (!function_exists('vc_publish_alert_redirect')) {
    function vc_publish_alert_redirect(string $message): void {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["status"], $_POST["semester"], $_POST["year"], $_POST["dte_publish"])) {
    ...
    if ($token === '' || !hash_equals((string)($_SESSION['csrf_token'] ?? ''), $token)) {
        vc_publish_alert_redirect('Security validation failed. Please refresh the page and try again.');
    }
    ...
    if (!in_array($status, ['0', '1'], true) || !in_array($semester, ['1', '2'], true) || !ctype_digit($year) || $dte_publish === '') {
        vc_publish_alert_redirect('Please provide valid publishing details.');
    }
    ...
    if ($insert) {
        ...
        if ($insert->execute()) {
            vc_publish_alert_redirect('Final exam results publishing date set successfully.');
        }
    }

    vc_publish_alert_redirect('Something went wrong while setting the final results publishing date.');
}
?>
