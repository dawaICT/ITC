...
if(!empty($_POST)){
      if(isset($_POST["course_code"], $_POST["course_name"])) {
        ...
        $checkStmt = $db->prepare("SELECT course_code FROM courses WHERE course_code = ? AND course_name = ? LIMIT 1");
        $index = null;
        if ($checkStmt) {
            $checkStmt->bind_param("ss", $course_code, $course_name);
            $checkStmt->execute();
        ...
        else  if (!empty($course_code) && !empty($course_name)) {
            $insert = $db->prepare("INSERT INTO courses (course_code, course_name) VALUE(?,?)");
...
<body>
  <div id="Course" class="w3-modal">
    <div class="w3-modal-content w3-animate-zoom w3-card-8">
      <header class="w3-container w3-blue"> 
        <span onclick="document.getElementById('Course').style.display='none'" 
      ...
      <div class="w3-container">
        <form action="add_courses.php" method="post" role="form">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="course_code" autofocus id="course_code" 
                   placeholder="Enter program code" autocomplete="off" required>

                </div>
                <div class="form-group">
                  <lable for="course_name">Course name:</lable><br>
