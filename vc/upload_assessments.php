...
if(!empty($_POST)){
      if(isset($_POST["SID"], $_POST["course_code"], $_POST["semester"], $_POST["assess_type"], $_POST["assess_num"], $_POST["marks"], $_POST["year"])) {
        ...
        $checkStmt = $db->prepare("SELECT SID FROM semester_registration WHERE SID = ? AND semester = ? LIMIT 1");
        $isRegistered = false;
        if ($checkStmt) {
            $checkStmt->bind_param("ss", $SID, $semester);
            $checkStmt->execute();
            $checkStmt->store_result();
            $isRegistered = $checkStmt->num_rows > 0;
            $checkStmt->close();
        }

        if (!$isRegistered) {
            echo "<script>alert('Ooops! Student ID not registered for this semester.')</script>";
