...
if(!empty($_POST)){
      if(isset($_POST["Sid"], $_POST["amount_paid"], $_POST["balance"], $_POST["narration"], $_POST["semester"], $_POST["Year"], $_POST["dte_time"])) {
        ...
        if (!empty($Sid) OR empty($amount_paid) OR empty($balance) && !empty($invoice) OR empty($narration) OR empty($semester) OR empty($Year) OR empty($dte_time)) {
            ...
            if ($insert->execute()) {
                            echo "<script>alert('Student Invoice Successful!')</script>";
              echo"<script>window.open('invoiceStudent.php','_self')</script>";

              }
            }
            else {
              echo "<script>alert('Student Invoice Failed!')</script>";
              echo"<script>window.open('invoiceStudent.php','_self')</script>";
          ...
          <div class="w3-card-4 w3-white"> 
            ...
            <div class="w3-container">
              <ul class="w3-navbar">
                ...
                <hr>
              <form role="form" method="POST" action="invoiceStudent.php" class="w3-row-padding">
