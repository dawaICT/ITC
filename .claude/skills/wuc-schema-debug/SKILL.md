...
- **PHP CLI** (most reliable for inspecting the DB): `C:\xampp\php\php.exe`
...
- The `mysql.exe` client (`C:\xampp\mysql\bin\mysql.exe`) exists but has been
...
Run from the project root (`C:\xampp\htdocs\wucportal`). Check whether a table
exists:

```bash
"C:\xampp\php\php.exe" -r "require_once 'db/connect.php'; \$r=\$db->query('SHOW TABLES LIKE \"payments\"'); echo (\$r && \$r->num_rows>0)?'EXISTS':'MISSING';"
```

List a table's real columns:

```bash
"C:\xampp\php\php.exe" -r "require_once 'db/connect.php'; \$r=\$db->query('DESCRIBE fee_structure'); while(\$row=\$r->fetch_assoc()) echo \$row['Field'].' '.\$row['Type'].\"\n\";"
```

Find candidate tables when a name is wrong:

```bash
"C:\xampp\php\php.exe" -r "require_once 'db/connect.php'; \$r=\$db->query('SHOW TABLES LIKE \"%pay%\"'); while(\$row=\$r->fetch_row()) echo \$row[0].\"\n\";"
```
...
| `programs.period_mode` | EXISTS as of 2026-07-02 (with `academic_structure`, `duration_value`, `duration_unit`, `uses_terms`, `uses_semesters`, `is_short_course`) — the old alias workaround is no longer needed; re-verify with DESCRIBE |
