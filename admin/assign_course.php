...
if (!empty($_POST) && isset($_POST["course_code"], $_POST["staff_id"])) {
    ...
    if (!empty($course_code) && !empty($staff_id)) {
        ...
        if (!empty($assignResult['inserted']) || !empty($assignResult['updated'])) {
            $_SESSION['successMsg'] = (string)$assignResult['message'];
        } else {
            $_SESSION['errorMsg'] = (string)($assignResult['message'] ?: 'This course is already assigned to the selected lecturer under all mapping contexts.');
        }
