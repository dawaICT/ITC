...
// Eligibility UI helpers for courseReg.php
window.WUC = window.WUC || {};
window.WUC.eligibility = (function(){
  function banner(message, type){
    var el = document.getElementById('eligibility-banner');
    if(!el){ return; }
  ...
  function listFailed(failed){
    var container = document.getElementById('failed-courses-list');
    if(!container){ return; }
    container.innerHTML = '';
    if(!failed || !failed.length){
      container.innerHTML = '<div class="text-muted">No failed courses detected.</div>';
      return;
    }
    ...
    failed.forEach(function(f){
      ...
      li.textContent = f.course_code + ' (grade ' + f.grade_letter + ')';
  ...
  return { banner: banner, listFailed: listFailed };
})();