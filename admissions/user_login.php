...
$scope = $isStaffLogin ? 'admissions-staff' : 'admissions-student';
if (wuc_is_login_locked($db, $scope, $username)) {
    legacy_login_fail('Too many failed attempts. Please try again in 15 minutes.');
}

if ($isStaffLogin) {
    $stmt = $db->prepare('SELECT id, staff_id, Fname, Lname, password, role, status FROM staff WHERE staff_id = ? OR email = ? LIMIT 1');
    if (!$stmt) {
...
$stmt = $db->prepare('SELECT sl.Sid, sl.Password, s.Fname, s.Lname, s.status FROM student_login sl LEFT JOIN students s ON s.SID = sl.Sid WHERE sl.Sid = ? LIMIT 1');
if (!$stmt) {
    legacy_login_fail('Login service temporarily unavailable.');
}
