...
if (!empty($_POST)) {
    if (isset($_POST["Sid"], $_POST["program_code"], $_POST["intake"], $_POST["mode"], $_POST["startYear"], $_POST["endYear"])) {
        ...
        $is_transfer = isset($_POST["isTransfer"]) ? 1 : 0;
        ...
        // Get term dates if provided
        $term_start_date = isset($_POST["term_start_date"]) ? trim($_POST["term_start_date"]) : null;
        $term_end_date = isset($_POST["term_end_date"]) ? trim($_POST["term_end_date"]) : null;

        if ($is_transfer) {
            $studentTransfer = $db->prepare("UPDATE students SET is_transfer = 1, transfer_from = ?, transfer_credits = ? WHERE SID = ?");
        ...
        $entryYear = (int)substr($startYear, 0, 4) ?: (int)date('Y');
        ...
        // Validate intake
        $term_based = isTermBasedProgram($program_code, $db);
        $valid_intakes = $term_based ? $intake_types['term'] : $intake_types['semester'];
        
        if (!in_array($intake, $valid_intakes)) {
            echo "<script>alert('Invalid intake selected for this program.')</script>";
            $valid = false;
        } else {
            if(isset($_GET['debug'])) {
                error_log("DEBUG: Processing admission for SID: $Sid");
            }

            // Check if student is already admitted to this program
            $check = $db->prepare("SELECT Sid FROM student_program WHERE Sid = ? AND program_code = ?");
            $check->bind_param("ss", $Sid, $program_code);
            $check->execute();
            ...
            if ($check_rs) {
                if(isset($_GET['debug'])) {
                    error_log("DEBUG: Student $Sid already admitted to program $program_code");
                }
                echo "<script>alert('Student ID " . htmlspecialchars($Sid) . " is already admitted to this program.')</script>";
                echo "<script>window.open('admitEnrolled_student.php','_self')</script>";
                die();
            }

            // Check if SID exists in students table
            $check_student = $db->prepare("SELECT SID FROM students WHERE SID = ?");
            $check_student->bind_param("s", $Sid);
            $check_student->execute();
            $student_result = $check_student->get_result();
            $student_rs = $student_result->fetch_assoc();
            $valid = true;

            if (!$student_rs) {
                if(isset($_GET['debug'])) {
                    error_log("DEBUG: Student $Sid not found in students table");
                }
                echo "<script>alert('Student ID " . htmlspecialchars($Sid) . " not found in the system.')</script>";
                $valid = false;
            }

            if ($valid && !empty($Sid) && !empty($program_code) && !empty($intake) && !empty($mode) && !empty($startYear) && !empty($endYear)) {
                // Calculate term dates for term-based programs if not provided
                if ($term_based && (empty($term_start_date) || empty($term_end_date))) {
                    // Calculate term dates based on intake
                    $year = date('Y');
                    switch($intake) {
                        case 'Term1':
                            $term_start_date = $year . '-01-15';
                            $term_end_date = $year . '-04-15';
                            break;
                        case 'Term2':
                            $term_start_date = $year . '-05-15';
                            $term_end_date = $year . '-08-15';
                            break;
                        case 'Term3':
                            $term_start_date = $year . '-09-15';
                            $term_end_date = $year . '-12-15';
                            break;
                    }
                }
                ...
                $insert = $db->prepare("INSERT INTO student_program (Sid, program_code, intake, mode, startYear, endYear, term_start_date, term_end_date, status) VALUES (?,?,?,?,?,?,?,?, 'Active')");
                $insert->bind_param("ssssssss", $Sid, $program_code, $intake, $mode, $startYear, $endYear, $term_start_date, $term_end_date);

                if ($insert->execute()) {
                    // Update student status
                    $update_status = $db->prepare("UPDATE students SET status = 'active', enrollment_date = NOW() WHERE SID = ?");
                    $update_status->bind_param("s", $Sid);
                    $update_status->execute();
                    
                    if(isset($_GET['debug'])) {
                        error_log("DEBUG: Student admission successful for SID: $Sid");
                    }
                    echo "<script>alert('Student admission successful')</script>";
                    echo "<script>window.open('students.php','_self')</script>";
                } else {
                    if(isset($_GET['debug'])) {
                        error_log("DEBUG: Process failed for SID: $Sid - " . $insert->error);
                    }
                ...
                if(isset($_GET['debug'])) {
                    error_log("DEBUG: Missing fields for SID: $Sid");
                }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="data-table-card border-0">
                    <div class="card-body">
                        <form action="admitStudent.php" method="post" id="admissionForm">
                            <div class="mb-3">
